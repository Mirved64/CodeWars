...
//
//   Your task is to return the number of people who are still on the
//   bus after the last bus stop (after the last array). Even though it is the last bus stop,
...
export const number = (busStops: [number, number][]): number => {
  const total = busStops.reduce((acc, el) => [acc[0] + el[0], acc[1] + el[1]])
